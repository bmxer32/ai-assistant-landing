...
В блоке «Демо бесплатно» — форма из двух полей и согласия на обработку данных.
Заявка уходит на `/ai/api/lead`, попадает в журнал на сервере и дублируется
в Telegram. Форма **работает без JavaScript**: обычный `<form method="post">`
уводит на `thanks.html`, с живым JS отправка идёт через `fetch` без перезагрузки.

Сервис приёма — в [`server/`](server/): Node без зависимостей, systemd-юнит,
установка и обслуживание описаны там же.

## Аналитика

Номер счётчика Яндекс.Метрики задаётся **в одном месте** — константа
`METRIKA_ID` в начале `script.js`. Пока там `0`, счётчик не подключается,
а вызовы целей молча ничего не делают.

Цели: `lead_form` (отправлена заявка), `tg_click`, `phone_click`,
`calc_used` (двигали калькулятор), `pricing_view` (долистал до тарифов).

Адрес входа в сессию запоминается в `sessionStorage` — чтобы utm-метки
и `yclid` не терялись, когда посетитель уходит с рекламной страницы
на кейсы и отправляет форму уже оттуда.
...
├── cases.html      кейсы
├── thanks.html     «заявка принята» — для отправки формы без JS
├── styles.css      все стили, токены в :root
├── script.js       чат-демо, калькулятор, вопросы, форма, Метрика
├── logo.jpg        логотип
...
├── server/         приём заявок: Node-сервис, systemd, nginx
├── avito/          обложки для объявлений: covers.html + render.js + PNG
└── cover1–4.html   первая версия обложек, системным шрифтом
    avito-cover-*.png   и они же в PNG
```

Актуальные обложки для Avito — в [`avito/`](avito/): шесть картинок в стиле
лендинга, собраны из одного исходника, там же тексты объявлений. Файлы
`cover1–4.html` в корне — предыдущая версия, свёрстанная системным шрифтом;
их можно удалить.
...
контакты, реквизиты, форма заявки и связь через Telegram [@webe9](https://t.me/webe9).

Перед запуском платного трафика осталось: вписать номер счётчика Метрики,
заполнить `/etc/lead-api/config.json` токеном бота и заменить демо-кейсы
на реальные.
